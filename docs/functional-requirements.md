...
- **機能概要**: OAuth プロバイダー経由での新しいユーザーアカウント作成
- **対応プロバイダー**: Google, GitHub
- **取得情報**:
  - メールアドレス（プロバイダーから自動取得）
  - 表示名（プロバイダーから自動取得、編集可能）
  - プロフィール画像（プロバイダーから自動取得）
- **成功条件**: OAuth 認証完了、ユーザーアカウント作成

#### 0.2 ユーザーログイン（SSO）

- **機能概要**: OAuth プロバイダー経由での既存ユーザー認証
- **認証手段**: OAuth (Google, GitHub)
- **セッション**: JWT + Database セッションのハイブリッド
- **セッション維持**: 30 日間（プロバイダー設定に依存）
- **セキュリティ**: OAuth プロバイダーのセキュリティポリシーに依存

#### 0.3 アカウント管理

- **機能概要**: OAuth アカウント情報の管理
- **管理項目**:
  - 表示名の編集
  - プロフィール画像の変更
  - 連携済みプロバイダーの確認
- **注意**: パスワードリセット機能は SSO では不要
